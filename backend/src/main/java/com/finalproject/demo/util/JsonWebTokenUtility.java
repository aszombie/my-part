...
public class JsonWebTokenUtility {
	...
	public void init() {
		// TODO：應該實作從資料庫抓出密鑰
		// String secret = "ABCDEFGHJKLM23456789npqrstuvwxyz";//sean
		String secret = "oiwehaf123apdo567isfhasdijoaei123456sfjdsopifhnak";// aszombie
		// 將密鑰使用base64編碼
	...
	public String createEncryptedToken(String data, Long lifespan) {
		...
		JwtBuilder builder = Jwts.builder().subject(data) // JWT主體內容
				.issuedAt(now) // 建立時間
				.expiration(expiredate) // 過期時限
				.encryptWith(password, // 加密：避免內容被讀取
						Jwts.KEY.PBES2_HS512_A256KW, Jwts.ENC.A256GCM);
	...
	public String validateEncryptedToken(String token) {
		// 建立密碼
		Password password = Keys.password(charArraySecret);
		JwtParser parser = Jwts.parser().decryptWith(password) // 解密：以便讀取內容
				.build();
		try {
	...
	public String createResetPassWordToken(String data, Long lifespan) {
		java.util.Date now = new java.util.Date();
		if (lifespan == null) {
			lifespan = 3L * 60 * 1000;
		}
			...
			JwtBuilder builder = Jwts.builder().subject(data) // JWT主體內容
					...
					.expiration(expiredate) // 過期時限
					.signWith(secretKey); // 使用密鑰簽章：避免內容被竄改
			String resetPassWordtoken = builder.compact();
	...
	public String validateToken(String token) {
		// 使用HMACS-SHA演算法建立簽章密鑰
		SecretKey secretKey = Keys.hmacShaKeyFor(base64EncodedSecret);
		JwtParser parser = Jwts.parser().verifyWith(secretKey) // 使用密鑰驗證簽章：避免內容被竄改
				.build();
		try {
